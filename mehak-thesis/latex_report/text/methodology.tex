...
Formal CA2 requires a Google Cluster Trace pipeline with Acc/Prec/Rec/F1/ROC-AUC/latency against hybrid and traditional monitors. That pipeline is delivered via \texttt{src/data/gct\_loader.py} on a disclosed 2011 subset (fail-closed if parts are missing; see \texttt{DATA\_GAPS.md} and \texttt{DESIGN\_RATIONALE\_BEYOND\_CA2.md}). Section~\ref{subsec:gct} summarises the GCT windowing used for formal scores; Section~\ref{subsec:synth} documents the synthetic generator retained only for development. Sections~\ref{subsec:window}--\ref{subsec:training} describe the shared history/future forecasting design (originally exercised on synthetic data; GCT uses the same 10+5 window lengths). Section~\ref{subsec:metrics} lists the formal metric suite.

\subsection{Google Cluster Trace (formal CA2)}
\label{subsec:gct}

Official ClusterData 2011 parts under \texttt{data/gct/2011/} (4 event + 2 usage parts in the scored subset) are joined on $(\mathrm{job\_id},\mathrm{task\_index})$. Windows use a 10-step history and 5-step horizon; FAIL/EVICT/KILL/LOST in the future window force unhealthy labels. Fail-series-first sampling yields 981 FAIL-forced windows of $N{=}12000$. The MHSA fourth channel is \textbf{sampled CPU}: ClusterData 2011 has no network-byte field (\texttt{CHANNEL\_HONESTY.md}; not invented). Expanded Aldomi features add usage columns plus history-only SCHEDULE/UPDATE counts. Full remaining 2011 parts and 2019 Borg cells are scoped-out optionals, not floor blockers.

\subsection{Synthetic Telemetry Generator (artefact-only harness)}
\label{subsec:synth}

A synthetic telemetry generator (\texttt{src/data/telemetry\_simulator.py}) remains for local, reproducible development of the MHSA code path. It is \textbf{not} formal CA2 evidence: binding scores use \texttt{--dataset gct}. The generator produces controlled multivariate series with known cross-metric burst precursors so architecture and metric wiring can be exercised offline.
...
Both attention models (MHSA-PerHead and MHSA-Fused) are trained with the Adam optimizer (learning rate 0.001, default $\beta_1 = 0.9$, $\beta_2 = 0.999$). On formal GCT runs: 15 epochs, $N{=}12000$ windows, seeds 42--46. The synthetic harness (development only) uses 30 epochs on 20,000 samples per seed. Splits are 80/20 train/test.
