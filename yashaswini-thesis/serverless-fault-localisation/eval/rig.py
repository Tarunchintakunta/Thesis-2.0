...
def write(res: dict, windows: dict, out: Path, figs: Path, source: str, ceiling_f1: float = 0.938) -> dict:
    ...
    text = ["# Rig results - rule arm (detection + localisation)", "", "Generated by `eval/rig.py`.", "",
            ...
            f"Xing et al. (2025) report detection F1 {ceiling_f1:.3f} with labelled training data on their own "
            f"benchmark. The rule arm's F1 here is {f1:.3f}; the {100 * (ceiling_f1 - f1):.1f} pp gap is a positioning "
            "against that ceiling, not a same-rig comparison.", "", SIGN, ""]
