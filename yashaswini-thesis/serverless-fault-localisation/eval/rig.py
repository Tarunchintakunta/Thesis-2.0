...
def write(res: dict, windows: dict, out: Path, figs: Path, source: str, ceiling_f1: float | None = None) -> dict:
    out.mkdir(parents=True, exist_ok=True)
    ...
    if ceiling_f1 is None:
        ceiling = (
            f"Measured rule-arm detection F1 on this rig is {f1:.3f}. "
            "No literature F1 is hard-coded into the report; cite Xing et al. separately if needed."
        )
    else:
