...
\begin{enumerate}
    ...
    \item \textbf{Leg 3 (Overhead):} Measure telemetry volume, latency, and cost on live AWS --- \textbf{not executed}; arithmetic estimates only
\end{enumerate}
...
Policy-sampling arithmetic (synthetic; Leg~3 live AWS absent) estimates $\sim$87\% volume and $\sim$95\% cost reduction vs full tracing. These figures \textbf{cannot} certify the overhead half of the competitiveness rule without live measurement.

\subsection{Competitiveness Decision}

Under the pre-committed rule (F1 within 10\,pp \textit{and} volume $\geq$50\% cut), competitiveness is \textbf{not supported}: Leg~2 F1/Top-3 evidence fails the accuracy reading; Leg~3 live overhead is missing.
...
\begin{itemize}
    ...
    \item Overhead ``87\% / 95\%'' cuts remain synthetic until Leg~3 is run
\end{itemize}
...
Rig results are from simulated telemetry, not actual CloudWatch/X-Ray data. RCAEval raw already shows rules Top-3 $=0.611$ (far below sim $0.979$). Live AWS Leg~3 would additionally be needed for overhead; detection F1 under real noise is unknown and must not be assumed from sim ($0.97$).
...
Committed evidence does \textbf{not} support a within-10\,pp competitiveness win: RCAEval rules Top-3 $=0.611$ vs BARO/CIRCA $=0.878$, rule-arm F1 $=0.469$, and Leg~3 live AWS overhead is absent. The three-leg protocol remains useful for positioning future methods once live overhead and complete baselines are in place. Xing et al.\ (2025) stays a Leg~1 citation ceiling only.
