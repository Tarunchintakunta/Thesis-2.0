...
End-to-end load-generator latency (policy vs off): Mann--Whitney $p\approx1.2\times10^{-5}$, median difference $\approx -42$\,ms (policy lower). List-price cost per million requests from measured volumes: full $\approx\$10.93$, policy $\approx\$2.25$. Learned lower-bound column omitted (RCAEval parquet absent on the runner) --- not invented.
