...
Confirmatory experimental results in this submission are generated from the local simulator (`DRY\_RUN=1`). A **lite live AWS round** (4 cells $\times n{=}1$, 200 orders, destroyed after) is reported under \texttt{results/live/key\_cells/}. Live$\leftrightarrow$sim \textbf{relative ranks} agree; absolute recovery seconds are not scale-matched. Live confirmatory $n{>}1$ remains future work.
...
\begin{itemize}
    ...
    \item \textbf{Simulated confirmatory campaigns}, plus a measured live lite 4-cell smoke (not confirmatory $n{>}1$).
\end{itemize}
