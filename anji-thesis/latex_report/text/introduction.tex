...
Confirmatory hypothesis tests in this submission are generated from the local simulator (`DRY\_RUN=1`). A separate \textbf{live AWS lite} key-cell round (4/4 cells; $n{=}1$; destroy-after-round) is reported in Chapter~5; it is directional only and does not replace localsim H1--H3. Chapter~5 also records a beyond-CA2 localsim $n{=}3$ pass on those four cells and the live $n{=}3$ plan that was not executed under shared-account concurrency limits. Absolute live timings may differ from simulation due to network variability and cold starts.
...
\begin{itemize}
    ...
    \item \textbf{Simulated confirmatory experiments} (localsim H1--H3) plus a \textbf{live lite} 4-cell AWS round (directional; $n{=}1$).
\end{itemize}
