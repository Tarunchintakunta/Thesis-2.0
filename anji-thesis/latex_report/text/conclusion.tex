...
\begin{itemize}
    \item \textbf{External validity (AWS fidelity):} Lite live 4-cells show \textbf{relative ranks} matching localsim (VT, MRC DLQ/dup). Absolute recovery is shorter on the 200-order live cells than on $\sim$3600-order localsim campaigns. Confirmatory live $n{>}1$ is still open.
...
\begin{enumerate}
    \item \textbf{Live AWS Validation:} Lite 4/4 + relative-rank fidelity is done. Remaining: confirmatory repeats / CA2-depth key cells (VT 600 / MRC 10 / $n{>}1$). Do not start a new apply by default.
