...
\begin{itemize}
    ...
    \item \textbf{MRC$\rightarrow$recovery H2:} fail to reject ($H=4.98$, $p_{\mathrm{adj}}=0.520$). Exploratory DLQ capture vs MRC is the clear signal (MRC\,1 mean $\approx0.294$ under unhandled\_error vs $\approx0$ at MRC$\ge5$).
    \item \textbf{H3\_recovery rejects H0 after Holm} ($U=250$, $p_{\mathrm{adj}}=3.17\times10^{-3}$, $r=0.667$): throughput-favourable VT\,600/batch\,50 mean recovery $600$\,s vs alternatives mean $224$\,s. Do not cite withdrawn $p_{\mathrm{adj}}=0.084$ drafts.
\end{itemize}
