...
This is the first systematic, quantitative study of SQS configuration under controlled, reproducible fault injection. Prior work by \citet{Kyrychenko2025} established steady-state performance optima; this research extends that baseline to failure scenarios, revealing that:

\begin{itemize}
    \item \textbf{Zero message loss} is achievable with DLQ + sufficient visibility timeout and maxReceiveCount (H1 supported).
    \item \textbf{Recovery time follows visibility timeout nearly 1:1} (600s VT → 600s recovery), identifying VT as a recovery time knob, not just a latency parameter.
    \item \textbf{maxReceiveCount determines DLQ vs. duplicate trade-off} for transient faults (H2 supported): low MRC dead-letters unnecessarily, high MRC increases duplicates.
    \item \textbf{Steady-state throughput optima (long VT, large batch) incur 16× longer recovery time} (H3 large effect), suggesting practitioners must balance performance vs. resilience.
\end{itemize}
...
\begin{itemize}
    ...
    \item 350+ committed run manifests with seeds, configs, and metrics for replication and extension.
\end{itemize}
...
\begin{description}
    ...
    \item[Idempotency:] Mandatory for at-least-once delivery. Higher MRC amplifies duplicates (14\% additional at MRC 10); idempotent handlers neutralize this.
\end{description}
...
The study validates simulation as a cost-effective, reproducible alternative to live experimentation for relative configuration effects. Absolute timing differs from AWS (no cold starts, deterministic timing), but relative rankings (VT 600 > VT 30 for recovery time) are expected to hold. This approach enables research that would be cost-prohibitive on live infrastructure (\$50-100+ for 350 runs).
...
\begin{itemize}
    \item \textbf{External validity (AWS fidelity):} Absolute metrics may differ from live AWS due to cold starts, network jitter, CloudWatch delays. Relative effects are expected to hold, but live validation (future work) is needed to quantify deltas.
...
\begin{enumerate}
    \item \textbf{Live AWS Validation:} Execute a subset of runs (20–30 key cells) on AWS, compare metrics to simulator, quantify fidelity. Publish delta report as appendix. Estimated cost: \$10-15.
