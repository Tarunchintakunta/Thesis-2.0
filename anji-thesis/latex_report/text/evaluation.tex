...
\textbf{Directional only (not confirmatory $n{>}1$):} under consumer\_kill, measured recovery was longer at VT\,90 than VT\,30; under unhandled\_error, MRC\,1 showed DLQ capture $0.16$ while MRC\,5 showed DLQ $0$ with duplicate rate $0.05$; loss was $0$ on all four cells. Live$\leftrightarrow$sim \textbf{relative ranks agree} (\texttt{results/live/key\_cells/fidelity.json}; VT90 matched to sim VT60). Absolute recovery seconds are \textbf{not} comparable (live 200 orders vs localsim $\sim$3600). Do \textbf{not} claim live H1--H3 decisions or completion of the burst/\texttt{key\_cells.yaml} matrix (VT\,600 / MRC\,10 / repeats\,5) from this lite round.
...
\begin{itemize}
    ...
    \item \textbf{Live lite 4/4 + relative-rank fidelity:} measured smoke on AWS; confirmatory/deeper CA2 key-cell live still open.
\end{itemize}

Confirmatory findings are \textbf{localsim}. Adaptive VT/DIVE was not evaluated (\texttt{adaptive\_vt: false}). Cost was not an experimental DV. Live AWS lite evidence exists; live$\leftrightarrow$sim relative ranks agree. CA2 alignment remains \textbf{below 100\%} pending confirmatory or deeper live key-cell coverage.
