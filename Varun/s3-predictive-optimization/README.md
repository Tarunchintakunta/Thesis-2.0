...
│   ├── metadata/                     # Inventory CSV / ListObjects / lite reconstruct
...
**Not present as live evidence (do not claim):** live Inventory job, multi-workload Wilcoxon campaign, CE-settled savings. Module `src/metadata/collector.py` and `analysis/statistics.py` exist.
...
- Live Boto3 metadata collector **implemented** (`src/metadata/collector.py`); live Inventory job **not** enabled; lite reconstruct in `results/live/lite_metadata.json`
