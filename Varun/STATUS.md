...
**Research alignment:** ~93% — dry-run evidence locked; live lite S3/CE/CW/Wilcoxon probe executed and destroyed; `src/metadata/` collector + reconstructed lite inventory + FinOps notes present.  
**Not SUBMIT-READY.** Residual: full multi-workload live FinOps campaign (live Inventory job, settled CE savings, multi-workload Wilcoxon protocol).
...
| `src/metadata/` collector | **Present** — ListObjects/Inventory CSV + lite-round reconstruct (`results/live/lite_metadata.json`, 48 objects). Live Inventory *job* never enabled |
...
1. Live S3 Inventory **job** never enabled (collector module exists; lite reconstruct is not a live ListObjects after destroy).
2. Settled Cost Explorer validation of campaign savings (CE lag; lite ≠ savings trial).
3. Multi-workload Wilcoxon success protocol (helper exists; lite is single-bucket probe).
...
2. **AWS residual:** full live S3 FinOps evaluation (Inventory job + multi-workload Wilcoxon + CE-settled costs). Collector/notes closed as non-AWS.
