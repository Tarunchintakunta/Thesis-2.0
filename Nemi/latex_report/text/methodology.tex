...
All experiments report both a local simulation (reproducible 30-round campaigns) and a live AWS lite federated round (one Free-Tier \texttt{t3.micro}, S3, CloudWatch; destroyed after measurement):

\begin{itemize}
    \item \textbf{Local hardware:} Standard development machine (CPU-only)
    \item \textbf{Live lite:} AWS \texttt{t3.micro} in \texttt{eu-west-1} (\texttt{results/live/cloud\_lite\_summary.json})
    \item \textbf{Software:} Python 3.9+, PyTorch 2.0+, scikit-learn
    \item \textbf{Simulation:} Multi-process federated clients with shared test set; live round uses the same in-process clients on EC2 with S3 weight round-trip
