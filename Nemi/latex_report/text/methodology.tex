...
\textbf{Dataset actually used in the committed PoC:} a \textbf{synthetic} UNSW-NB15-\emph{derived} feature sample (20 features; limited sample size; binary labels), generated locally when the full CSV is unavailable. Committed metrics are in \texttt{results/comparison/results.json} (30 rounds, 5 clients).

\textbf{UNSW-NB15} \citep{moustafa2015unsw} is the \emph{intended} full benchmark (2.5M flows, 49 features, 9 attack types). A full UNSW-NB15 (or stratified 50k) experiment is \textbf{not} committed and must not be described as executed.
