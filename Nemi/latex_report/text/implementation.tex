...
IaC (live lite + destroy) & Terraform (EC2+S3+CloudWatch) \\
Version Control & Git \\
\bottomrule
\end{tabular}
\caption{Technology stack for SecureFL-IDS (30-round campaigns local; live lite on AWS)}
...
\begin{itemize}
    \item Local simulation for 30-round campaigns; live AWS lite FL on one \texttt{t3.micro} measured then destroyed
    \item Synthetic dataset when full UNSW-NB15 unavailable
    \item CPU-only testing (GPU optional but not required)
    \item Lite cloud clients are in-process on one node (not WAN-distributed)
\end{itemize}
