...
\begin{itemize}
    ...
    \item Matrix dimensions beyond the local evidence set, \textbf{after} a true multi-node EC2 campaign confirms crossover
...
Guidance only (no measured EC2 timings in this deliverable):

\begin{itemize}
    \item Prefer validating scale-up on one large instance before assuming multi-instance benefit
    \item Do not treat local Dask times as AWS EC2 results
    \item Matched aggregate vCPU EC2 runs remain required to answer the CA2 cloud-instance RQ
\end{itemize}
...
\begin{itemize}
    \item \textbf{Matrix size range}: Local suite covers 200--2000; \textbf{no crossover} in that range; AWS multi-node still missing
    \item \textbf{Local execution}: True multi-node network latency not captured; local Dask $\neq$ matched-vCPU EC2
...
\begin{enumerate}
    \item \textbf{Larger Matrices / Cloud}: Run matched-vCPU AWS EC2 campaign (CA2 residual); do not extrapolate local crossover
...
This work provides local-proxy evidence for a decision that cloud practitioners face routinely. It does \textbf{not} claim a measured AWS crossover. By documenting that local distributed never beats threading through 2000×2000 in committed JSON, and by separating CA2 EC2 requirements from LocalCluster practice, this study:
