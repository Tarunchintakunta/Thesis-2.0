...
\begin{itemize}
    ...
    \item \textbf{Live EC2 round-1:} matched 2-vCPU topology — \textbf{1× t3.small} (scale-up) vs \textbf{2× t3.micro} (scale-out) in \texttt{eu-west-1}; artefact \texttt{results/live/ec2\_round1\_summary.json}; fleet destroyed after the round. Terraform defaults match this topology.
\end{itemize}

Local matched cores on one host are still \textbf{not} a substitute for multi-instance network cost. Live round-1 evidence:
