...
\begin{itemize}
    ...
    \item \textbf{Live EC2 round-2:} same topology; \texttt{ec2\_round2\_summary.json}; multi-instance \texttt{da.matmul} completed at $n{=}250$ (0.01824\,s); fleet destroyed.
\end{itemize}
...
Local matched cores on one host are still \textbf{not} a substitute for multi-instance network cost. Live round-1+2 evidence:
    ...
    \item Multi-instance: round-1 futures smoke OK / \texttt{da.matmul} timed out; round-2 \texttt{da.matmul} \textbf{0.01824\,s} at $n{=}250$

\begin{itemize}
    \item Scale-up numpy matmul ($n{=}500$): $\approx$0.0067\,s on \texttt{t3.small}
    \item On-node Dask \texttt{LocalCluster} per \texttt{t3.micro} ($n{=}500$): mean $\approx$0.52\,s
    \item Multi-instance: two workers registered; futures smoke OK; \texttt{da.matmul} \textbf{timed out} (no completed multi-instance matmul cell)
    \item CLI exposes \texttt{--scheduler-address} for the remote Dask arm
\end{itemize}
