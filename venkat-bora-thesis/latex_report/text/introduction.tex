...
\begin{enumerate}
    ...
    \item \textbf{Reproducible Framework}: A benchmarking harness with pytest coverage, automated metric collection, and repeated local iterations (\texttt{n\_iterations: 3} in committed JSON).
...
Performance is measured via completion time (wall-clock), peak resident memory (RSS), and CPU utilization (sampled percentage). Each configuration is repeated three times in the committed campaign; descriptive mean$\pm$std values are reported from \texttt{summary\_statistics.json}.

\textbf{Important:} The committed factorial suite uses local Dask \texttt{LocalCluster} on one host. Matched-vCPU AWS EC2 rounds (1× \texttt{t3.small} vs 2× \texttt{t3.micro}) are recorded in \texttt{results/live/ec2\_round1\_summary.json} and \texttt{ec2\_round2\_summary.json}; round-2 completed multi-instance \texttt{da.matmul} at $n{=}250$ (0.2546\,s).
...
\begin{itemize}
    ...
    \item \textbf{Chapter 6 (Evaluation)}: Experimental results aligned to committed JSON, local-only limitations, and discussion of findings.
