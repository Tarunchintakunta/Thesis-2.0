...
\textbf{Important:} The committed factorial suite uses local Dask \texttt{LocalCluster} on one host. A matched-vCPU AWS EC2 round-1 (1× \texttt{t3.small} vs 2× \texttt{t3.micro}) is recorded in \texttt{results/live/ec2\_round1\_summary.json}; multi-instance \texttt{da.matmul} timed out and is not claimed as a completed timing.
