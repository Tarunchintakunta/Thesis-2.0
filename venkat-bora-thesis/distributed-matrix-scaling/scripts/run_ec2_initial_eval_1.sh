#!/usr/bin/env bash
# initial_eval_1: ONE confirmatory matched-vCPU EC2 gate (eu-west-1).
# Topology: 1× t3.small (scale-up) vs 2× t3.micro (scale-out). Destroy after.
# Saves under results/live/initial_eval_1/. Does NOT start final-3.
set -euo pipefail
...
NAME_PREFIX="${NAME_PREFIX:-matrix-scale-ie1}"
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
OUTDIR="${OUTDIR:-$ROOT/results/live/initial_eval_1}"
...
multi = {
    ...
    "note": "Scheduler on scale-out-0; one dask-worker per t3.micro (2 workers total); initial_eval_1 gate.",
}
if dist_raw is None:
...
echo "initial_eval_1 complete; destroy confirmed"
