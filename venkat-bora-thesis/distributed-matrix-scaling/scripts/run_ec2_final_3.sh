#!/usr/bin/env bash
# final_3: ONE confirmatory matched-vCPU EC2 gate (eu-west-1).
# Topology: 1× t3.small (scale-up) vs 2× t3.micro (scale-out). Destroy after.
# Saves under results/live/final_3/. Does NOT start final-3.
set -euo pipefail
...
NAME_PREFIX="${NAME_PREFIX:-matrix-scale-f3}"
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
OUTDIR="${OUTDIR:-$ROOT/results/live/final_3}"
...
multi = {
    ...
    "note": "Scheduler on scale-out-0; one dask-worker per t3.micro (2 workers total); final_3 gate.",
}
if dist_raw is None:
...
echo "final_3 complete; destroy confirmed"
