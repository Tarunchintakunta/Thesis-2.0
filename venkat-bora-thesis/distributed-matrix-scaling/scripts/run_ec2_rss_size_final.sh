#!/usr/bin/env bash
# rss_size_final: RQ limbs — peak RSS/CPU + size ladder. Usage: bash scripts/run_ec2_rss_size_final.sh <1|2|3>
ROUND="${1:?round 1|2|3 required}"
...
NAME_PREFIX="${NAME_PREFIX:-matrix-rss-f${ROUND}}"
...
OUTDIR="${OUTDIR:-$ROOT/results/live/rss_size_final_${ROUND}}"
...
for ID in "$UP" "$SCHED" "$WORK"; do
  aws ssm send-command --region "$REGION" --instance-ids "$ID" \
    --document-name AWS-RunShellScript --timeout-seconds 60 \
    --parameters "commands=[\"mkdir -p /opt/matrix-scale\",\"echo $QB_B64 | base64 -d > /opt/matrix-scale/quick_bench.py; echo $DB_B64 | base64 -d > /opt/matrix-scale/dist_bench.py\",\"ls -la /opt/matrix-scale/quick_bench.py\"]" \
...
summary = {
    ...
    "ami_id": ami,
    "name_prefix": "matrix-rss-f${ROUND}",
    "destroy_after": True,
}
...
echo "VENKAT_RSS_SIZE_FINAL_${ROUND}_DONE $(date -u +%Y-%m-%dT%H:%M:%SZ); destroy confirmed"
