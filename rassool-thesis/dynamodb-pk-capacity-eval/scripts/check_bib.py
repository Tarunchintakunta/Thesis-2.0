...
Rules from the master prompt section 4: only the 21 listed scholarly items
(2022-2026) plus the 3 AWS documentation pages, every entry with a DOI or URL,
baseline Pantelic et al. (2026) present.
"""
...
BASELINE_DOI = "10.3390/fi18010053"
...
def main(argv=None) -> int:
    ...
    if not any(e.get("doi", "").lower() == BASELINE_DOI for e in entries):
        problems.append("baseline Pantelic et al. (2026) missing")
    for e in entries:
        if not (e.get("doi") or e.get("url")):
            problems.append(f"{e['key']}: no DOI or URL")
    print(f"{len(entries)} entries ({len(scholarly)} scholarly + {len(entries) - len(scholarly)} AWS docs)")
    if not args.offline:
        for e in entries:
            try:
                ok, why = resolve(e)
