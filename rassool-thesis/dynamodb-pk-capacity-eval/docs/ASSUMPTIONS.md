...
| I3 | Every resource is tagged `project=dynamodb-pk-capacity`, `managed_by=terraform`, `purpose=research-eval`, `data=synthetic` through the provider's `default_tags`. **No student name or student ID in Terraform.** | Master prompt 3.2 + TERRAFORM_POLICY. |
...
| D5 | Zipf exponent **s = 1.070916**, calibrated so the hottest 10% of the 1M keys get 90.0% of operations (top 1%: 78%; the single hottest order: 10.65%). Ranks are mapped to orders through a fixed random permutation (seed 20250920). | Master prompt 2.2 asks for ~90/10 and forbids uniform access. |
