...
\begin{enumerate}
    \item \textbf{Empirical evidence on configuration-level trade-offs:} This is, to the author's knowledge, the first peer-reviewed controlled experiment simultaneously varying partition-key design and capacity mode on a production cloud-metered database service (DynamoDB), quantifying the dependent variables—throttling, consumed capacity, and absolute cost—that self-hosted baselines cannot produce. The study retains the latency and throughput metrics of \citet{pantelic2026benchmarking} for comparability, while extending the evaluation to metered dimensions.
